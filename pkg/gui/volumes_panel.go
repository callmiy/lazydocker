...
func (gui *Gui) getVolumesPanel() *panels.SideListPanel[*commands.Volume] {
	return &panels.SideListPanel[*commands.Volume]{
		...
		GetFilterIdentities: volumeFilterIdentities,
		GetItemKey:          func(volume *commands.Volume) string { return volume.Name },
	}
}

func volumeFilterIdentities(volume *commands.Volume) []panels.FilterIdentity {
	return []panels.FilterIdentity{{Value: volume.Name, DisplayCell: 1}}
}
...
func (gui *Gui) handleVolumesBulkCommand(g *gocui.Gui, v *gocui.View) error {
	bulkCommands := make([]config.CustomCommand, 0, 1+len(gui.Config.UserConfig.BulkCommands.Volumes))
